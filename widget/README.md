...
프레임리스·투명·드래그 이동·핀(항상-위)·최소화/닫기가 되는 빈 위젯 창.
데이터(해빗/타임블록/타이머) 연결은 W1 이후.

| Phase | 내용 |
|---|---|
| **W0** ← 지금 | Tauri 스캐폴드, 프레임리스/투명/핀 창 |
| W1 | Firebase 로그인 + 해빗 위젯(읽기) |
| W2 | 위치 영속·트레이·자동시작 |
| W3 | 타임블록 위젯 |
| W4 | 타이머 위젯(+쓰기) |
| W5 | 웹앱 위젯 설정 패널 |
...
## 동작 확인 (W0)
- 위젯 창이 **테두리 없이 둥근 카드**로 뜬다 (투명 배경).
- 상단 바를 **드래그**하면 창이 이동한다.
- **📌** 누르면 다른 창 위로 고정(다시 누르면 해제), 버튼이 강조된다.
- **—** 최소화, **×** 닫기.
- 데이터 영역엔 "위젯 스캐폴드 (W0)" 표시.
- `src/index.html`을 일반 브라우저로 열면 "브라우저 미리보기 (Tauri 밖)" — 레이아웃만 확인용.
