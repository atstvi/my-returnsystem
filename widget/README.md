...
## 현재 상태: W3 (멀티창 — 해빗 위젯 + 타임블록 위젯)

**앱을 따로 열지 않아도 윈도우 부팅 시 두 위젯이 자동으로 뜬다.** ×를 눌러도 종료가
아니라 트레이로 숨겨져 백그라운드에서 계속 돌고, 트레이 아이콘으로 다시 띄울 수 있다.
창 위치/크기도 각각 기억한다. **기능당 창 하나씩** — 해빗 창과 타임블록 창이 분리된다.
