...
- 첫 실행 시 **Google로 로그인** 버튼이 표시된다.
  - 누르면 위젯 창 안에서 Google 로그인 페이지로 전환(`signInWithRedirect`)됐다가
    인증 후 위젯으로 되돌아온다.
  - 인증 정보는 로컬 IndexedDB에 저장돼 다음 실행부터 자동 로그인된다.

> **인증 구조 메모.** 위젯 프론트엔드는 `tauri://` 가 아니라 바이너리에 내장된
> 초소형 HTTP 서버를 통해 **`http://localhost:14317`** 로 서빙된다. Firebase
> `signInWithRedirect` 는 인증 후 앱 origin 으로 되돌아와야 하는데, firebaseapp.com
> 인증 핸들러는 커스텀 스킴(`tauri://`)으로는 리다이렉트하지 못한다. `localhost` 는
> Firebase 인증 허용 도메인이라 이 방식이라야 로그인이 완료된다(`src/lib.rs`
> `start_local_server`, `tauri.conf.json` window.url, capabilities `remote.urls`).
