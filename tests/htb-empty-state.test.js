'use strict';
/* 나탭 타임블록 — 오늘 시간 지정 블록이 하나도 없으면(종일만 있거나 범위 밖으로 빠졌거나
   계획이 없거나), 큰 빈 격자로 홈을
...
t.ok('시간 블록 없으면(종일만 있어도) htb-empty 클래스', /if\(!_htbHasTimed\)\{\s*_htbBody\.classList\.add\('htb-empty'\);/.test(html));
t.ok('종일만 있을 때 안내 문구 적응', /_htbHadAllday\?'시간을 정한 계획이 없어요':'오늘은 아직 시간 계획이 없어요'/.test(html));
