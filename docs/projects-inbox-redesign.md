...
## H. 디테일 마무리 — 폴더 타일 + 전체 타임라인
- **폴더 타일**: 상단 프로젝트 색 액센트 바 + 메타 `할일 N · pct% · 마감 M/D · D-day`(구분점 정합·마감
  포맷·D-day) + 프로젝트 색 진행바(mini) + 최근 로그.
- **전체 타임라인 정합**: 요일 헤더(`.pj-tl-days`)에 본문 그리드와 같은 좌우 패딩(`sp-4`) → 막대가 요일
  칼럼과 정확히 정렬. *(헤드리스: 장보기=수, 스튜디오 예약=목, 믹싱 마감 목→토 span, pageerror 0.)*
...
- **연결선(커넥터) — 구현됨**: `board.edges=[{id,from,to}]`. 툴바 **🔗 연결** 토글 → 두 항목 클릭으로
  엣지 생성(중복/자기연결 방지). world-space SVG(`#pjb-edges`)로 렌더 → pan/zoom·드래그 실시간 추종.
  일반 모드에서 선 클릭 시 삭제(굵은 투명 히트라인). 항목 삭제 시 관련 엣지 자동 정리.
  *(헤드리스 검증: 연결 모드→두 항목→엣지 1 렌더·영속, pageerror 0.)*
...
- **LLM 자동분류(구현됨)**: ✨ 자동 → `callAI`(Groq)로 카테고리 id 분류(키 없거나 실패/미매칭 시
  키워드 휴리스틱으로 폴백). 분류 중 버튼 로딩 상태. *(헤드리스: 키 없음 → 폴백 buy 확인.)*
- **카테고리 순서 변경(구현됨)**: 칸반 컬럼 헤더 그립(⠿) 포인터 드래그로 순서 변경 → INBOX_CATS 순서
  saveCats 영속(기기 간). *(헤드리스: task를 3번째로 드래그 → [idea,task,buy,…] 영속, pageerror 0.)*
