...
## H. 설정 모달 + 알림 정리 + 기록 토글 + 주간 여백 (오너 피드백, 원띵 참고)

- **재생 옆 알림 버튼 정리** — 뜻이 불명확하던 🔔(알림 권한)을 액션 행에서 제거. 알림 권한은
  **설정 모달의 "세션 완료 알림" 행**으로 이동(권한 상태 라벨 + 켜기). 유휴 시 우측 슬롯은 spacer로
  균형 유지, 세션 중에는 ■ 중단만.
- **타이머 설정 = 버튼 → 창** — 인라인 드로어를 없애고 **`⚙ 타이머 설정` 버튼**이 모달(`.ft-modal`,
  §3.5)을 연다. `focusTimerSettingsRows`/`wireFocusTimerSettings`/`focusTimerOpenSettings`.
- **기록 기본 숨김 + 타임라인 길게** — 오늘 존에 **`[타임라인 | 기록]` 페인 토글**(`data-ft-pane`,
  `focusTodayPane`). 기본은 타임라인이 존 전체 높이를 채워(`flex:1`) 더 길고 깔끔. 기록은 토글 시 같은
  영역에 전체 목록으로 표시. `+ 직접 추가`는 토글 행 우측.
- **주간 기록 좌우 여백** — 전면 그리드가 끝에 붙던 걸 `.ftl-wrap` 좌우 패딩 `--sp-12` + `max-width
  1400` 중앙정렬로 밀도 확보.
...
## K. 카테고리 편의 + 할일 검색 + 드래그 추가 (오너 피드백)

`focusTimerOpenTargetPicker`를 **통합 피커**로 확장(target/add/drag 모드):
- **할일 검색** — 세부 섹션에 `할일 검색…` 인풋(돋보기). 전체 열린 할일을 라이브 필터(부분일치, 60개
  표시). 검색어 입력 시 목록만 부분 렌더해 포커스 유지.
- **카테고리 필수** — 라벨에 `필수` 배지, 카테고리 미선택 시 저장/추가 버튼 `disabled`. 또 **시작(▶)
  누를 때 카테고리 없으면 피커를 먼저 열고**(`startAfter`) 저장 후 자동 시작.
- **기록 색 = 카테고리 색** — 카테고리 필수화로 새 세션은 항상 카테고리 색(타임라인·기록·주간). 옛
  기록은 모드색 폴백 유지.
- **타임라인 드래그 추가** — 오늘 타임라인 빈 영역을 드래그하면 고스트 블록으로 범위를 그리고, 놓으면
  그 시각·길이가 채워진 **집중 기록 추가** 시트가 열림(`_ftWireTimelineDrag`, 마우스/펜; 터치는 스크롤
  유지). `+ 직접 추가`/`+ 기록 추가` 버튼도 같은 add 모드(날짜·시각·분 입력 + 카테고리 필수 + 검색)로 통일.

## L. 프리셋 타이머 (원띵 참고, 3단계)

자주 쓰는 `카테고리 + 모드 + 시간(+세부)`을 **프리셋**으로 저장 → **원탭 시작**.
- 저장소 `focus_presets_v1`(디바이스 로컬). `focusPresetLoad/Save/AddCurrent/Apply/NewId`.
- 콘솔 타깃 칩 아래 **프리셋 칩 행**: `[● 이름 · 25분]` … `＋ 저장`. 탭 = 설정 적용 + 즉시 시작
  (`focusPresetApply`; 진행 중이면 토스트로 막음). `＋ 저장` = 현재 카테고리/모드/시간을 프리셋으로
...
## N. 완료 흐름 회복 — 모달 → 액션 스낵바 (오너 피드백 "정작 할일 완료가 안됨")

- **문제**: §M에서 할일 완료마다 전체화면 확인 모달(`openConfirmDialog`)이 떠서 할일을 연속으로
  빠르게 체크하는 흐름이 끊겼다("완료가 안 되는" 느낌). 완료 자체는 되지만 매번 모달이 가로막음.
- **수정**: `focusTimerOfferRecord`가 모달 대신 **비차단 액션 스낵바**(`focusOfferToast`)를 띄운다.
  완료는 즉시 처리되고, 하단(네비 위 76px)에 `[할일명] 집중 기록으로 [추가]` 알약이 슬라이드업 →
  5초 후 자동 사라짐. `추가` 버튼만 `pointer-events` 활성, 탭하면 기존 `focusTimerAddFromTask`로
  add 시트를 연다(프리필 동일). 새 완료가 오면 이전 스낵바를 교체(한 번에 하나). 연속 완료가 전혀
  막히지 않음 → "모든것이 간편하고 금방".
