...
| T6 | 보조액션 배치 | 헤더에 뷰스위처+4버튼이 몰림, 검색행은 항상 펼쳐져 밀도↑ | 뷰스위처(주)와 보조 아이콘 정리, 검색/필터는 **검색 버튼 눌렀을 때** 노출(현재 항상 보임) | P1 |
| T7 | 빠른추가 툴바 | 칩이 **이모지**(🔥🏷️📅), 홈 캡처바와 톤 다름 | 칩 아이콘 SVG화, §3.7 톤과 정렬(액센트 포커스 등) | P2 |
| T8 | 대량작업 바 | 버튼 이모지(✓↩📅🗑), 스타일 제각각 | `.btn`류로 통일, 삭제=danger(§3.1) | P2 |
| T9 | 빈/로딩 상태 | 카테고리 0개·검색결과 0·완료숨김 등 상태 편차 | §3.8 빈상태 패턴 일괄 | P1 |
