...
| T7 | 빠른추가 툴바 | 칩이 **이모지**(🔥🏷️📅), 홈 캡처바와 톤 다름 | 칩 아이콘 SVG화, §3.7 톤과 정렬(액센트 포커스 등) | P2 ✅(우선순위·카테고리=색 점, 마감=캘린더 SVG; `.add-chip-label`만 갱신해 아이콘 유지) |
| T8 | 대량작업 바 | 버튼 이모지(✓↩📅🗑), 스타일 제각각 | `.btn`류로 통일, 삭제=danger(§3.1) | P2 ✅(라인 SVG 아이콘, 삭제=danger 유지) |
| T9 | 빈/로딩 상태 | 카테고리 0개·검색결과 0·완료숨김 등 상태 편차 | §3.8 빈상태 패턴 일괄 | P1 ✅(빈 날=프롬프트+CTA, 빈 카테고리=흐리게; 검색결과 0은 모달 `.empty-state` 유지) |
| T10 | 파괴적 확인 | 이름삭제/완료삭제 등 일부 native `confirm()` | `openConfirmDialog`(§3.5) | P2 ✅(bulkDelete/bulkDeleteDone/openNameDelete/단일삭제 → danger 다이얼로그+confirm 폴백. 취미탭 hobDeleteItem/deleteCat은 취미탭 차례에) |
